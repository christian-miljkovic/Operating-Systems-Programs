...
def highest_penalty_ratio(process_array):
	...
	while(all_terminated == False):
		...
		if(len(process_queue) > 0):
			...
			#check which one has the highest penalty ratio and make that the first one on the queue to run
			


			for i in range(0,len(process_queue)):

				if(len(process_queue) == i):
					break
				...
				#if we havent popped the done process in first index we change the next process behind it to running
				#assuming that the previous run we have index 0 running and index 1 had been changed to ready
				elif((len(process_queue) > 1) and ((process_queue[0].current_state =="blocked") or (process_queue[0].current_state =="terminated")) and (made_change == False)):

					#pdb.set_trace()
					max_ratio = 0
					#use this for when looking for the queue
					index = 0
					new_array = []
					which_array = "queue"

					#see which has the highest penalty ratio looking first at the processes in the process_queue
					for j in range(1,len(process_queue)):

						if(process_queue[j].current_state == "ready"):

							t = 0

							if((process_queue[j].original_cpu_time - process_queue[j].cpu_time) == 0):
								t = 1
							else:
								t = process_queue[j].original_cpu_time - process_queue[j].cpu_time


							process_ratio = (process_queue[j].wall_clock_time/t)
							#print(process_queue[j].toString(),process_queue[j].index,process_ratio)


							if(process_ratio > max_ratio):
								max_ratio = process_ratio
								index = j

					for m in range(0,len(blocking_list)):

						if(blocking_list[m].i_o_burst == 1):

							#pdb.set_trace()


							if((blocking_list[m].original_cpu_time - blocking_list[m].cpu_time) == 0):
								
								t = 1
							else:
								
								t = (blocking_list[m].original_cpu_time - blocking_list[m].cpu_time)

							
						
							process_ratio = (blocking_list[m].wall_clock_time/t)
							#print(blocking_list[m].toString(),blocking_list[m].index,process_ratio)


							if(process_ratio >= max_ratio):
								max_ratio = process_ratio
								index = m
								which_array = "blocking"


					new_running_process = None

					#check which array to use
					if(which_array == "blocking"):

						new_running_process = blocking_list[index]
						blocking_list.remove(blocking_list[index])

					else:

						new_running_process = process_queue[index]
						



					new_running_process.current_state = "running"


					#compute the cpu burst
					#calculate the randomOS time for the single burst
					new_running_process.cpu_burst = randomOS(new_running_process.b_value)

					#if the burst is greater than the cpu time left set it equal to that
					if(new_running_process.cpu_burst > new_running_process.cpu_time):
						new_running_process.cpu_burst = new_running_process.cpu_time

					#calcualte the I/O burst
					new_running_process.i_o_burst = new_running_process.cpu_burst * new_running_process.multiplier


					new_array.append(process_queue[0])
					new_array.append(new_running_process)

					for l in range(0,len(process_queue)):

						if((process_queue[l] in new_array) == False):
							new_array.append(process_queue[l])

					process_queue = new_array
					made_change = True
		...
		#update all of the wall clock times
		for i in range(0,len(process_array)):

			#we already added wall clock time at time zero to avoid division by zero
			if(process_array[i].current_state != "unstarted" and (cycle != 0)):
				process_array[i].wall_clock_time += 1
		
		
		cycle += 1
		#pdb.set_trace()
	print("The scheduling algorithm used was Highest Penalty Ratio")
	print("")
	for i in range(0,len(process_array)):
		process_array[i].summary()
	full_summary(process_array,num_process)
